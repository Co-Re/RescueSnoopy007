...
```{r hist,fig.align='center', fig.width=10,  fig.height = 4, echo=FALSE}
numeric_values = aac_intakes_outcomes[c(7, 8, 11, 12, 30, 31, 34, 35, 15, 16, 41, 18, 38, 39 ) ]

par(mfrow=c(2, 4), mar=c(3, 2, 3, 1) + 0.1)

colnames <- dimnames(numeric_values)[[2]]
for (i in 1:ncol(numeric_values)) {
    hist(numeric_values[,i], main=colnames[i], probability=TRUE, col="gray", border="white", xlab = "", cex.main=1.2)
    d <- density(numeric_values[,i])
    lines(d, col="red")
}
par(mfrow = c(1,1))
```
...
Um die bei den Decision Trees erreichten Accuracies zu verbessern, sollten Random Forests aufgestellt werden. Diese haben den Vorteil, dass sie noch recht nah an Decision Trees sind und dadurch noch vergleichsweise leicht interpretierbar sind. Im Gegensatz zu den Decision Trees wir diesesmal OOB und nicht CV5 verwendet. Das hat den Grund, dass der Datensatz sehr viele Variablen hat und OOB bei der Erstellung eines Random Forrest performanter ist.
...
Eine weitere Variable die noch genauer betrachtet hätte werden können, ist `breed`. Diese wurden in den Decision Trees als nicht relevant identifziert und darum in den Neuronalen Netzen nicht mehr verwendet. Jedoch könnte die Relevanz des Features durch Zusammenfügen mehrere Rassen zu einer Kategorie weitere Erkenntnisse liefern. Zum Beispiel ob Rassekatze eher adoptiert werden als nicht Rassekatze. Um diese einzelnen Kategorien der Rassen zu definieren, wäre jedoch noch eine tiefergehende Recherche im Bereich der Tierassen nötig bzw. wäre es am besten einen Experte auf diesem Gebiet miteinzubeziehen. 
