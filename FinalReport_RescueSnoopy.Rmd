...
### Visuelle Darstellung der Datenstrukturen

<details>
  <summary>Code Details</summary>
```{r hist}
qplot(aac_intakes_outcomes$age_upon_outcome_.days., geom="histogram")
qplot(aac_intakes_outcomes$age_upon_intake_.days., geom="histogram")
qplot(aac_intakes_outcomes$age_upon_outcome_.years., geom="histogram", binwidth=0.5)
qplot(aac_intakes_outcomes$age_upon_intake_.years., geom="histogram", binwidth=0.5)
qplot(aac_intakes_outcomes$time_in_shelter_days, geom="histogram")
qplot(aac_intakes_outcomes$outcome_month, geom="histogram", binwidth=0.5)
qplot(aac_intakes_outcomes$intake_month, geom="histogram", binwidth=0.5)
qplot(aac_intakes_outcomes$outcome_year, geom="histogram", binwidth=0.5)
qplot(aac_intakes_outcomes$intake_year, geom="histogram", binwidth=0.5)
qplot(aac_intakes_outcomes$outcome_hour, geom="histogram", binwidth=0.5)
qplot(aac_intakes_outcomes$intake_hour, geom="histogram", binwidth=0.5)
qplot(aac_intakes_outcomes$outcome_number, geom="histogram", binwidth=0.5)
qplot(aac_intakes_outcomes$intake_number, geom="histogram", binwidth=0.5)
qplot(aac_intakes_outcomes$dob_year, geom="histogram", binwidth=0.5)
qplot(aac_intakes_outcomes$dob_month, geom="histogram", binwidth=0.5)
```
</details>

Interpretation der Daten:

* Die Verteilung der Daten in den Variablen `age_upon_outcome` und `age_upon_intake` ist relativ ähnlich. Es werden kaum ältere Tiere ins Tierheim gebracht sowie aus dem Tierheim entnommen. Ein Großteil der Tiere ist sowohl bei der Einlieferung als auch bei der Entlassung jünger als ca. 1 Jahr. Hier ist auch zu sehen, dass es negative Altersangaben gibt (hierauf wird später noch Bezug genommen). Außerdem ist ersichtlich, dass die Altersangabe in Jahren ab einem ALter von ca. 1,5 auf eine Zahl ohne Nachkommazahlen gerundet wird. Bei der Betrachtung der Altersangabe in Tagen ist ersichtlich, dass es sowohl bei den Intakes als auch den Outcomes fehlende Werte an denselben Stellen gibt. Woher dies resultiert, sollte mit dem Tierheim abgeklärt werden.
...
* Die Verteilung über die Monate ist relativ gleichmäßig. 
...
* Die Verteilung der Variable `outcome_hour` zeigt, dass ab ca. 6 Uhr Tiere aus dem Tierheim entlassen werden. Diese Zahlen steigen - mit Ausnahme von 9 - 10 Uhr - an, bis der Höhepunkt von 16 bis 17 Uhr erreicht ist. 
* Die Verteilung der Variable `intake_hour` zeigt eher das Gegenteil von `outcome_hour`. Recht wenig Tiere werden vor 10 Uhr ins Tierheim gebracht, dann erfolgt ein recht rasanter Anstieg auf den Höhepunkt zwischen 10 und 11 Uhr. Anschließend flacht es langsam ab.
* Sowohl die `outcome_number` als auch die `intake_number` zeigen fast nur Werte unter 4 und hierbei auch nur einen minimalen Anteil größer 1.
...
* Die Verteilung von `dob_month` ist recht gleichmäßig, es gibt lediglich kleine Ausschläge zwischen März, April und Mai.
...
In der Korrelationsmatrix sind u.a. folgende Korrelationen erkennbar:
...
* Selbiges gilt für das `intake_year` und das `outcome_year`. Hier ist die Korrelation sogar noch stärker. Dies zeigt, dass die meisten Tiere wohl im selben Jahr in das Tierheim kommen sowie wieder aus dem Tierheim kommen.
* Zwischen den Monat des Intakes sowie Jahres des Outcomes und andersrum scheint es eine leicht negative Korrelation zu geben.
* Das Geburtsjahr korreliert stark negativ mit dem Alter bei Intake.
...
Um das Modell an unterschiedlichen Stichproben zu testen, verwenden wir die „trainControl“-Methode. Cross Validation wurde ausgewählt, da das Ziel ist, eine bestmögliche Performance des Modells auf neue Daten zu erhalten: Ziel: bestmögliche Generalisierung. Um die Datenmenge des Testsets zu vergrößern, wurde die Anzahl an Folds auf 5 gesetzt. Dadurch soll eine bessere Accuracy bei unbekannten Daten erreicht werden, um keine falschen Klassifizierungen der Tiere zu erreichen.
...
Um die bei den Decision Trees erreichten Accuracies zu verbessern, sollten Random Forests aufgestellt werden. Diese haben den Vorteil, dass sie noch recht nah an Decision Trees sind und dadurch noch vergleichsweise leicht interpretierbar sind. 
...
### Modell für Katzen
Der Random Forest mit OOB konnte beispielhaft für ein reduziertes Subsample mit 1000 Cat-Datensätzen gespeichert werden, wird aber nicht weiter herangezogen.
```{r eval=FALSE}
...
Die Bäume dienen als gut interpretierbare Modelle für ein besseres Verständnis der Adoptionsgründe. Nun soll noch getestet werden, ob ein Modell, dass stärker auf Vorhersage als auf Interpretierbarkeit abzielt, helfen kann, die Modellaccuracy noch weiter zu steigern. 
...
#Diskussion
Zur Verbesserung der Ergebnisse hätte der `outcome_subtype` miteinbezogen werden können. Hierdurch hätten die verwendeten Outcome-Types genauer aufgeteilt werden können und ggfs. eine bessere Prediction erreicht werden können. Da jedoch die Fachkenntnis in diesem Bereich teilweise zu wenig vorhanden war, wurde diese Variable nicht berücksichtigt. Ferner ist zu beachten, dass je weiter man die Variable aufgeteilt hätte, desto geringer wäre der Datensatz je Outcome gewesen und die Gefahr von Overfitting steigt. 
