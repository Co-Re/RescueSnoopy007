...
```{r}
plot(aac_intakes_outcomes2$outcome_type)

as.numeric(aac_intakes_outcomes2$animal_type)
...
# Allgemeiner Train Test Split
```{r}

train_function <- createDataPartition(
  y = aac_intakes_outcomes2$outcome_type,
  p = .70,
  list = FALSE
)

training <- aac_intakes_outcomes2[ train_function,]
testing  <- aac_intakes_outcomes2[-train_function,]
...
#nur 30% aller Datensätze von Hunden, um die Datenmenge zu verkleinern
train_function <- createDataPartition(
...
dogs_reduced <- dogs[ train_function,]


#Splitte 30% wiederrum in Test/Training
train_function <- createDataPartition(
  y = dogs_reduced$outcome_type_adopt,
  p = .70,
  list = FALSE
)

training_dog <- dogs_reduced[train_function,]
testing_dog  <- dogs_reduced[-train_function,]
```
...
seed_val = 123
vars1 = c("breed", "intake_condition", "dob_year", "time_in_shelter_days", "color_new", "outcome_type_adopt")

vars2 = c("breed", "intake_condition", "intake_type", "dob_year", "time_in_shelter_days", "sex_upon_outcome_second", "color_new", "outcome_type_adopt")
```

### Decion Tree für Hunde
```{r}
training_dog$outcome_type_adopt <- as.factor(training_dog$outcome_type_adopt)
training_dog_sub1 = training_dog[vars1]
training_dog_sub2 = training_dog[vars2]
...
tree_dog1 <- train(
  outcome_type_adopt ~ .,
  data = training_dog_sub1,
  trControl = cv_5,
  method = "rpart")

tree_dog2 <- train(
  outcome_type_adopt ~ .,
  data = training_dog_sub2,
  trControl = cv_5,
  method = "rpart")
...
```{r}
testing_dog$outcome_type_adopt <- as.factor(testing_dog$outcome_type_adopt)
testing_dog_sub1 = testing_dog[vars1]

pred_tree_dog1 <- predict(tree_dog1, testing_dog_sub1)
conf_matrix_tree_dog1 <- confusionMatrix(pred_tree_dog1, testing_dog_sub1$outcome_type)
conf_matrix_tree_dog1

testing_dog_sub2 = testing_dog[vars2]

pred_tree_dog2 <- predict(tree_dog2, testing_dog_sub2)
conf_matrix_tree_dog2 <- confusionMatrix(pred_tree_dog2, testing_dog_sub2$outcome_type)
conf_matrix_tree_dog2
```

Der Tree (tree_dog2) schneidet mit 0.744 etwas besser als tree1 mit 0.7205 ab.

```{r}
rpart.plot(tree_dog2$finalModel)
```


Der Tree zeigt, dass zuerst time_in_shelter eine Rolle spielt und somit die erste Abzweigung im Baum ist. Als weitere Variable verwendet der Baum intake_type und zwar ob Pulbic Assist.

### Decion Tree für Katzen
```{r}
training_cat$outcome_type_adopt <- as.factor(training_cat$outcome_type_adopt)
training_cat_sub1 = training_cat[vars1]
training_cat_sub2 = training_cat[vars2]
```
...
tree_cat1 <- train(
  outcome_type_adopt ~ .,
  data = training_cat_sub1,
  trControl = cv_5,
  method = "rpart")

tree_cat2 <- train(
  outcome_type_adopt ~ .,
  data = training_cat_sub2,
  trControl = cv_5,
  method = "rpart")
...
```{r}
testing_cat$outcome_type_adopt <- as.factor(testing_cat$outcome_type_adopt)
testing_cat_sub1 = testing_cat[vars1]
testing_cat_sub2 = testing_cat[vars2]

pred_tree_cat1 <- predict(tree_cat1, testing_cat_sub1)
conf_matrix_tree_cat1 <- confusionMatrix(pred_tree_cat1, testing_cat_sub1$outcome_type)
conf_matrix_tree_cat1

pred_tree_cat2 <- predict(tree_cat2, testing_cat_sub2)
conf_matrix_tree_cat2 <- confusionMatrix(pred_tree_cat2, testing_cat_sub2$outcome_type)
conf_matrix_tree_cat2
```

Der zweite Tree mit einer Accuracy 0,7818 ist etwas besser.

```{r}
rpart.plot(tree_cat2$finalModel)
```


Bei Katzen ist auch time_in_shelter_days die erste Split Variable des Baumes, dann kommt time_in_shelter. Aber auch das Geschlecht und der Zustand des Tieres sind im Baum als Variablen aufgeführt.

### Decion Tree für Hasen
```{r}
training_rabbit$outcome_type_adopt <- as.factor(training_rabbit$outcome_type_adopt)
training_rabbit_sub1 = training_rabbit[vars1]
training_rabbit_sub2 = training_rabbit[vars2]
...
tree_rabbit1 <- train(
  outcome_type_adopt ~ .,
  data = training_rabbit_sub1,
  trControl = cv_5,
  method = "rpart")

tree_rabbit2 <- train(
  outcome_type_adopt ~ .,
  data = training_rabbit_sub2,
  trControl = cv_5,
  method = "rpart")
...
```{r}
testing_rabbit$outcome_type_adopt <- as.factor(testing_rabbit$outcome_type_adopt)
testing_rabbit_sub1 = testing_rabbit[vars1]
testing_rabbit_sub2 = testing_rabbit[vars2]

pred_tree_rabbit1 <- predict(tree_rabbit1, testing_rabbit_sub1)
conf_matrix_tree_rabbit1 <- confusionMatrix(pred_tree_rabbit1, testing_rabbit_sub1$outcome_type)
conf_matrix_tree_rabbit1

pred_tree_rabbit2 <- predict(tree_rabbit2, testing_rabbit_sub2)
conf_matrix_tree_rabbit2 <- confusionMatrix(pred_tree_rabbit2, testing_rabbit_sub2$outcome_type)
conf_matrix_tree_rabbit2
```
Der zweite Tree ist bezüglich Accuracy um 4% (0,81) besser als der erste Tree.


```{r}
rpart.plot(tree_rabbit2$finalModel)
```


Bei Hasen ist auch die erste Variable des Baumes time_in_shelter_days, jedoch ist dann die zweite Split Variable das Geschlecht des Tieres.
