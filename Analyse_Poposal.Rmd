...
# Erzeugung eines Sub-Datasets
Filterung auf Vögel
Mit der droplevels()-Funktion werden nichtbenutzte Factor-Level entfernt.
Außerdem wird die Variable animal_type entfernt, da wird hier nur noch ein Factor-Level haben.

```{r}
aac_intakes_outcomes_bird <- aac_intakes_outcomes2 %>% filter(animal_type == "Bird")
aac_intakes_outcomes_bird <- droplevels(aac_intakes_outcomes_bird)
aac_intakes_outcomes_bird <- aac_intakes_outcomes_bird[,c(1,3:23)]
table(aac_intakes_outcomes_bird$outcome_type)

```
Filterung auf Katzen

```{r}
aac_intakes_outcomes_cat <- aac_intakes_outcomes2 %>% filter(animal_type == "Cat")
aac_intakes_outcomes_cat <- droplevels(aac_intakes_outcomes_cat)
aac_intakes_outcomes_cat <- aac_intakes_outcomes_cat[,c(1,3:23)]
table(aac_intakes_outcomes_cat$outcome_type)

```


# Train Test Split (nur für Vögel)
```{r}
train_function_bird <- createDataPartition(
  y = aac_intakes_outcomes_bird$outcome_type,
  p = .70,
  list = FALSE
)
training_bird <- aac_intakes_outcomes_bird[ train_function_bird,]
testing_bird  <- aac_intakes_outcomes_bird[-train_function_bird,]
```

# Decision Tree (für Vögel)
...
predictions_tree_bird <- predict(tree_cv5_bird, testing_bird)
confus.res_tree_bird <- confusionMatrix(predictions_tree_bird, testing_bird$outcome_type)
confus.res_tree_bird
```

# Train Test Split (nur für Katzen)
```{r}
train_function_cat <- createDataPartition(
  y = aac_intakes_outcomes_cat$outcome_type,
  p = .70,
  list = FALSE
)
training_cat <- aac_intakes_outcomes_cat[ train_function_cat,]
testing_cat  <- aac_intakes_outcomes_cat[-train_function_cat,]
```
...
predictions_tree_cat <- predict(tree_cv5_cat, testing_cat)
confus.res_tree_cat <- confusionMatrix(predictions_tree_cat, testing_cat$outcome_type)
confus.res_tree_cat
```
