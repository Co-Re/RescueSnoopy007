...
new_df_train = as.data.frame(training_rabbit[c(-1)])


# training_rabbit_nn <- as.data.frame(training_rabbit)
# testing_rabbit_nn <-  as.data.frame(testing_rabbit)
# 
# str(training_rabbit)

testing_rabbit$outcome_type_adopt

str(new_df_train)

net_model_rabbit = neuralnet(outcome_type_adopt~ .,data=new_df_train,hidden=c(10),linear.output=T, act.fct = "logistic")
net_model_rabbit
```

```{r}
pred_net_rabbit <- predict(net_model_rabbit, testing_rabbit[c(-1)])
conf_matrix_net_rabbit <- confusionMatrix(pred_net_rabbit, testing_rabbit[c(-1)]$outcome_type_adopt)
conf_matrix_net_rabbit
# confusionMatrix()
# pred_net_rabbit
```
