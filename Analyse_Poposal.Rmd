...
aac_intakes_outcomes2$breed <- factor(aac_intakes_outcomes2$breed)
...
Wenn der Hund weniger als 4 Tage im Tierheim ist, scheint eine Adoption auf Basis des Modells also als unwahrscheinlich, wohingegen bei Hunden, die länger als 4 Tage im Tierheim verbleiben, die Wahrscheinlichkeit einer Adoption dann hoch ist, wenn der Hund sterilisiert ist. 
...
Je Modell ist eine Tabelle der Wichtigkeit einer Variable sowie eine Grafik dargestellt. Die Tabelle enthält lediglich Variablen, bei denen die Importance größer 0.
...
Es ist ersichtlich, dass für den Decision Tree der Hunde lediglich 6 Prädiktoren eine Rolle spielen:
...
Am wichtigsten sind die Variablen `sex_upon_outcome_first_Sterilized` und `sex_upon_outcome_first_Intact.`
...
Hier ist im Gegensatz zu den Hunden die `time_in_shelter` am wichtigsten.
...
Hier sind neben den bekannten Prädiktoren u.a. noch die Prädiktoren
* `intake_year2016`
* `outcome_year2016`
* `intake_year2015`
* `outcome_month9`
...
## Neuronales Netzwerk
Bei den neuronalen Netzen wird die Breed Variable ausgeschlossen, da sie schon bei den Bäumen als nicht relevant identifiziert wurde. Außerdem werden die numerischen Variablen intake_number, age_upon_intake_.days, age_upon_outcome_.days und time_in_shelter_days vorab min-max normalisiert, um die Geschwindigkeit der Berechnung zu verbessern bzw. bei den Hundedatenset konnte keine Berechnung aus Computer Performance technischen Gründen durchgeführt werden. Mit Normalisierung konnte, dann jedoch eine Berechnung durchgeführt werden.
Es werden für jede Tierart jeweil zwei neuronale Netze erstellt, einmal mit allen Variabeln und einmal mit den Variablen die im Decision Tree schon als wichtig festgestellt wurden.
...
data_train_model <- training_cat
data_test_model <- testing_cat
...
```{r}
imp_bag = vi(baggedtree)
imp_bag = imp_bag[imp_bag$Importance>5,]
imp_bag_tab = tibble(Variable = imp_bag$Variable, Importance = imp_bag$Importance)
imp_bag_tab
vip(baggedtree, width = 0.5, fill = "green3")
```
...
```{r}
imp_rfcv = vi(randomforest_CV)
imp_rfcv = imp_rfcv[imp_rfcv$Importance>5,]
imp_rfcv_tab = tibble(Variable = imp_rfcv$Variable, Importance = imp_rfcv$Importance)
imp_rfcv_tab
vip(randomforest_CV, width = 0.5, fill = "green3")
```
...
# Modell für Katzen (Achtung, nur mit einem Subsample von n=1000!)
```{r}
...
```{r}
imp_rfoob = vi(randomforest_OOB)
imp_rfoob = imp_rfoob[imp_rfoob$Importance>5,]
imp_rfoob_tab = tibble(Variable = imp_rfoob$Variable, Importance = imp_rfoob$Importance)
imp_rfoob_tab
vip(randomforest_OOB, width = 0.5, fill = "green3")
```
...
tree3 = tibble(
  ...
  TrainAccuracy = max(randomforest_CV$results$Accuracy),
  TestAccuracy = conf_matrix_RF_CV$overall[1])

tree4 = tibble(
  Model = "Random Forrest OOB",
  TrainAccuracy = max(randomforest_OOB$results$Accuracy),
...
Laden der gespeicherten Neuronalen Netze Modelle
```{r}
...
```{r}
imp_nn = vi(net_model_dog)
imp_nn = imp_nn[imp_nn$Importance>5,]
imp_nn_tab = tibble(Variable = imp_nn$Variable, Importance = imp_nn$Importance)
imp_nn_tab
vip(net_model_dog, width = 0.5, fill = "green3")
```

Neuronales Netz mit time_in_shelter_days und sex_upon_intake_first_Sterilized
```{r}
set.seed(seed_val)
...
```{r}
imp_nn_s = vi(net_model_dog_selected)
imp_nn_s = imp_nn_s[imp_nn_s$Importance>5,]
imp_nn_s_tab = tibble(Variable = imp_nn_s$Variable, Importance = imp_nn_s$Importance)
imp_nn_s_tab
vip(net_model_dog_selected, width = 0.5, fill = "green3")
```
...
```{r}
pred_net_dog <- predict(net_model_dog,testing_dog[c(2:5,7:78)])
idx <- apply(pred_net_dog, c(1), maxidx)
pred_net_dog <-as.factor(c('0', '1')[idx])

pred_net_dog_selected <- predict(net_model_dog_selected,testing_dog[c(2:5,7:78)])
idx <- apply(pred_net_dog_selected, c(1), maxidx)
pred_net_dog_selected <-as.factor(c('0', '1')[idx])

cf_net_dog <- confusionMatrix(pred_net_dog,testing_dog$outcome_type_adopt)
cf_net_dog_selected <- confusionMatrix(pred_net_dog_selected,testing_dog$outcome_type_adopt)
...
```{r}
imp_nn_cat = vi(net_model_cat)
imp_nn_cat = imp_nn_cat[imp_nn_cat$Importance>5,]
imp_nn_cat_tab = tibble(Variable = imp_nn_cat$Variable, Importance = imp_nn_cat$Importance)
imp_nn_cat_tab
vip(net_model_cat, width = 0.5, fill = "green3")
```

Neuronales Netz mit sex_upon_outcome_first_Sterilized, Alter und time_in_shelter
```{r}
set.seed(seed_val)
...
```{r}
imp_nn_s_cat = vi(net_model_cat_selected)
imp_nn_s_cat = imp_nn_s_cat[imp_nn_s_cat$Importance>5,]
imp_nn_s_cat_tab = tibble(Variable = imp_nn_s_cat$Variable, Importance = imp_nn_s_cat$Importance)
imp_nn_s_cat_tab
vip(net_model_cat_selected, width = 0.5, fill = "green3")
```
...
Verlgeich der Accuracy der Modelle
```{r}
pred_net_cat <- predict(net_model_cat,testing_cat[,-6] )
idx <- apply(pred_net_cat, c(1), maxidx)
pred_net_cat <-as.factor(c('0', '1')[idx])

cf_net_cat <- confusionMatrix(pred_net_cat,testing_cat$outcome_type_adopt)

pred_net_cat_selected <- predict(net_model_cat_selected,testing_cat[,-6] )
idx <- apply(pred_net_cat_selected, c(1), maxidx)
pred_net_cat_selected <-as.factor(c('0', '1')[idx])

cf_net_cat_selected  <- confusionMatrix(pred_net_cat_selected,testing_cat$outcome_type_adopt)
...
```{r}
imp_nn_rab = vi(net_model_rabbit)
imp_nn_rab = imp_nn_rab[imp_nn_rab$Importance>5,]
imp_nn_rab_tab = tibble(Variable = imp_nn_rab$Variable, Importance = imp_nn_rab$Importance)
imp_nn_rab_tab
vip(net_model_rabbit, width = 0.5, fill = "green3")
```

Neuronales Netz mit sex_upon_outcome_first_Sterilized
```{r}
set.seed(seed_val)
...
```{r}
imp_nn_s_rab = vi(net_model_rabbit_selected)
...
imp_nn_s_rab_tab
vip(net_model_rabbit_selected, width = 0.5, fill = "green3")
```
...
Verlgeich der Accuracy der Modelle
```{r}
pred_net_rabbit <- predict(net_model_rabbit,testing_rabbit[c(2:5,7:78)])
idx <- apply(pred_net_rabbit, c(1), maxidx)
pred_net_rabbit <-as.factor(c('0', '1')[idx])

pred_net_rabbit_selected <- predict(net_model_rabbit_selected,testing_rabbit[c(2:5,7:78)])
idx <- apply(pred_net_rabbit_selected, c(1), maxidx)
pred_net_rabbit_selected <-as.factor(c('0', '1')[idx])

cf_net_rabbit <- confusionMatrix(pred_net_rabbit,testing_rabbit$outcome_type_adopt)
cf_net_rabbit_selected <- confusionMatrix(pred_net_rabbit_selected,testing_rabbit$outcome_type_adopt)
