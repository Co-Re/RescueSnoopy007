...
Wie auch bei der Datensatzerstellung je Tierart werden im Folgenden pro Tierart Klassifikationsbäume mit 5-fold Cross-Validation gefittet. Dafür werden alle Variablen im Datensatz und die Methode 'rpart' aus dem Caret Package verwendet, um eine Adoption vorherzusagen. 
...
*Interpretation*
Der Tree zeigt, dass zuerst `time_in_shelter_days` eine Rolle spielt und somit die erste Abzweigung im Baum ist. Als weitere Variable verwendet der Baum `sex_upon_outcome_first_ Sterilized` = 0.
...
Daher scheinen für die Adoption einer Katze ähnliche Kriterien eine Rolle zu spielen, wie bei Hunden. Im Gegensatz zur Wahrscheinlichkeit einer Adoption eines Hundes spielt diesem Klassifikationsbaum zufolge aber das Alter der Katze eine Rolle: Wenn die Katze sterilisiert, und jünger als ein halbes Jahr ist, ist eine Adoption wahrscheinlicher. Ist die Katze älter als ein halbes Jahr, wird sie eher nicht adoptiert (Wahrscheinlich ist eine Adoption nämlich nur dann, wenn sie bereits länger als 12 Tage dort war).
Dies könnte daran liegen, dass Jungtiere bevorzugt adoptiert werden. 

Im Vergleich zum ersten Tree für die Vorhersage einer Adoption eines Hundes, zeigt sich beim Entscheidungsbaum für Katzen ohne Optimierung bereits eine bessere bzw. höhere Accuracy von über 80%. Auffallend ist im direkten Vergleich der Modelle vor allem auch, dass bei Katzen (umgekehrt als bei Hunden), die Adoption besser vorhergesagt werden kann (Sensitivity = 94%), als Fälle, in denen Katzen nicht adoptiert werden (Specificity = 62%). 
...
*Interpretation*
Bei Hasen gibt es 4 Splits: `sex_upon_outcome_first_Sterilized` = 0, `outcome_month9` = 0, `time_in_shelter_days` < 4.5, `age_upon_outcom_.days.` >= 405.
...
Die Accuracy ist mit 91% noch besser als jene der anderen beiden Trees (Hunde und Katzen). 
...
Die Relevanz der Jahreszeiten lässt darauf schließen, dass die Adoptionen von Hasen sich über die Jahre hinweg unterscheiden. Was Grund für das Ausreißerjahr 2016 war, müsste anhand von Brancheninformationen ermittelt werden, es könnt z.B. sein, dass es vermehrte mediale Aufmerksamkeit (z.B. durch Kampagnen / Testimonials) oder eine Art "Haustiertrend-Boost" für Hasen in diesem Jahr gab. Sollten sich die besonderen outcome_come Jahre und Monate auf ein außergewöhnliches Ereignis zurückführen lassen, muss bewertet werden ob diese Daten im Model berücksichtigt werden sollen.
...
Die Accuracy des NNs für Katzen ist unter Berücksichtigung aller Variablen deutlich besser als unter Verwendung eines Subsets. Die Accuracy des Treemodels kann mit dem Neuronalen Netz durchaus um etwa 5% verbessert werden. 
...
Zur Ermittlung der Variable Importance wurde ein weiteres NN für Hasen mit allen Prädiktoren gefittet.
```{r nnet, results='hide'}
set.seed(123)
...
Die Accuracy des NNs für Hasen ist unter Berücksichtigung aller Variablen deutlich besser als unter Verwendung eines Subsets. Die Accuracy des Treemodels kann, wie auch bei den Katzen, mit dem Neuronalen Netz nicht bzw. nur gering verbessert werden. 


####Variablen Importance im NN

Da die in den Bäumen ermittelten Variablen mit hoher Wichtigkeit für das Modell stark von jenen für Hunde und Katzen abweichen, wurde noch ein weiteres Neuronales Net (andere Library, die auch Einsicht der Variablen Importance des NNs ermöglicht) gefittet, um zu prüfen, inwiefern die Relevanz der Variablen im Neuronalen Netz von jenen abweicht, die im Tree ermittelt wurden.

Neben der Sterilisation, die auch im Tree eine der wichtigsten Variable darstellt, spielten beim Neuralen Net vorrangig Monate und Jahre, in denen das Tier ins Tierheim gekommen ist bzw. das Tierheim verlassen hat, eine Rolle. Diese weichen leicht von den wichtigen Variablen ab, die im Tree als wichtig erachtet werden. Die Darstellung bestätigt aber jedenfalls nochmals, dass bei Hasen tatsächlich Saisoninformationen hilfreich dafür sind, die Adoption vorherzusagen.
...
```
Bei allen Netzen ist die Accuracy unter Verwendung aller Features höher, als unter Verwendung eines Subsets, obwohl bei den Modellen für Hasen kaum ein Unterschied merkbar ist. 
Wie auch bei den Bäumen ist die Accuracy für Hunde weiterhin am geringsten, konnte jedoch deutlich verbessert werden mit dem NN. 
