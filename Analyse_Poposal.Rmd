...
seed_val = 123
vars = c("breed", "intake_condition", "dob_year", "time_in_shelter_days", "color_new", "outcome_type_adopt")
```

### Decion Tree für Hunde
```{r}
training_dog$outcome_type_adopt <- as.factor(training_dog$outcome_type_adopt)
training_dog_sub = training_dog[vars]
...
testing_dog_sub = testing_dog[vars]

pred_tree_dog <- predict(tree_dog, testing_dog_sub)
conf_matrix_tree_dog <- confusionMatrix(pred_tree_dog, testing_dog_sub$outcome_type)
conf_matrix_tree_dog
```

```{r}
rpart.plot(tree_dog$finalModel)
```
Der Tree zeigt, dass vor allem time_in_shelter bei Hunden eine Rolle spielt. Als einzige weitere Variable ist intake_conditionNormal aufgeführt.

### Decion Tree für Katzen
```{r}
training_cat$outcome_type_adopt <- as.factor(training_cat$outcome_type_adopt)
training_cat_sub = training_cat[vars]
...
testing_cat_sub = testing_cat[vars]

pred_tree_cat <- predict(tree_cat, testing_cat_sub)
conf_matrix_tree_cat <- confusionMatrix(pred_tree_cat, testing_cat_sub$outcome_type)
conf_matrix_tree_cat
```

```{r}
rpart.plot(tree_cat$finalModel)
```
Bei Katzen ist auch time_in_shelter_days der wichtigste Wert. Hinzu kommt als Variable wie bei den Hunden intake_conditionNormal. Somit sind Hunde und Katzen in der Auswahl der Variablen sehr ähnlich.

### Decion Tree für Hasen
```{r}
training_rabbit$outcome_type_adopt <- as.factor(training_rabbit$outcome_type_adopt)
training_rabbit_sub = training_rabbit[vars]
...
testing_rabbit_sub = testing_rabbit[vars]

pred_tree_rabbit <- predict(tree_rabbit, testing_rabbit_sub)
conf_matrix_tree_rabbit <- confusionMatrix(pred_tree_rabbit, testing_rabbit_sub$outcome_type)
conf_matrix_tree_rabbit
```

```{r}
rpart.plot(tree_rabbit$finalModel)
```
Bei Hasen ist auch die erste Variable des Baumes time_in_shelter_days, jedoch ist dann die zweite Split Variable das Alter des Tieres.
