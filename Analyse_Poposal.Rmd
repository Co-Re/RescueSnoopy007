...
Vorhersage des Outcome-Types auf Basis der gegebenen Variablen. Die initale Idee war es, die Prädiktoren zu finden, anhand derer eine Rückgabe an den Besitzer, ein Transfer / eine Adoption, Euthanasie, Tod etc. möglichst genau (durch Vorhersage der Klasse) vorhergesagt werden können. Eine detaillierte Auseinandersetzung bzw. im Zuge des Preprocessings stellte sich heraus, dass für viele der angeführten Outcome-Types wenige Fälle vorhanden sind, sodass eine Modellierung ohne Overfitting schwierig erscheint. Deshalb wurde während des Projektes bzw. zu Ende des Preprocessings redefiniert bzw. eingeschränkt auf Tierarten mit mindestens n=500 und ausschließlich den Outcometype Adoption. 
...
```{r create new dataframe, include=FALSE}
...
Auf Basis der Breed Variable wird die fehlende Information der Tierrasse aufgefüllt. 
...
```{r recode animal_type, include=FALSE}
...
```{r check recodings2, include=FALSE}
...
```{r recode sterilized, include=FALSE}
...
## Prüfung ob statistische Modellierung möglich ist
...
```{r check numerics, include=FALSE}
...
```{r remove wild animals and dummycode adoption, warning=FALSE, message=FALSE, include = FALSE}
#entferne wildtiere
...
```{r check recoding2, include=FALSE, message=FALSE, warning=FALSE}
...
```{r recode dummies, warning=FALSE, message=FALSE, include=FALSE}
...
```{r model fitting rabbits, include= FALSE}
set.seed(seed_val)
...
```{r save decision trees, include=FALSE}
if (!file.exists("decision_trees.RData"))
...
```{r model fitting cats, include=FALSE}
...
```{r model fitting selected vars1, include=FALSE}
set.seed(seed_val)
...
```{r modell fitting rabbits all vars, include=FALSE}
set.seed(seed_val)
...
```{r model fitting selected vars2, include=FALSE}
set.seed(seed_val)
...
```{r save NN models, include=FALSE}
if (!file.exists("neuronale_netze.RData"))
