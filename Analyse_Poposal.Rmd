...
Folgende Kombinationen sind beim Geschlecht möglich:
...
* found_location - Diese Variable bietet keine gute Datenqualität, es sind nicht immer gleiche Daten erfasst (z.B. manchmal Ortsname, manchmal komplette Adresse).
* sex_open_intake - Diese Variable enthält die selben Daten wie `sex_open_outcome`.

```{r}
table(aac_intakes_outcomes$sex_upon_intake_second, aac_intakes_outcomes$sex_upon_outcome_second)
```

* outcome_subtype - Diese Variable enthält nur sehr wenige Informationen, diese sind nicht durchgängig je `Outcome_type` befüllt.
...
* outcome_monthyear - der Monat und das Jahr werden als einzelne Variablen  verwendet
* outcome_hour - Es werden auch zu ungewöhnichen Uhrzeiten Geschäftsfälle registriert werden
...
* intake_hour - Es werden auch zu ungewöhnichen Uhrzeiten Geschäftsfälle registriert werden
...
* outcome_number - Diese Variable enthält dieselben Informationen wie `intake_number`.
...
* age_upon_intake_group - Die Gruppierung ist anhand falscher Cluster, je nach Tierarten bedeutet Alter etwas anderes. 
* age_upon_outcome, age_upon_outcome_year - Diese Variablen enthalten dieselben Informationen wie `age_upon_outcome_days`.
* age_upon_outcome_group - Die Gruppierung ist anhand falscher Cluster, je nach Tierarten bedeutet Alter etwas anderes.
...
kable(count(others$breed), align = "l") %>%
...
new_animal_type <- gsub("(.*)?(Cold Water|American|Tropical)(.*)?", "Other", new_animal_type)
