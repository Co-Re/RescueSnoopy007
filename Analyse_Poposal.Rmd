...
training_dog_norm[c(2:5)] <- as.data.frame(lapply(training_dog[c(2:5)], normalize))
...
```{r}
plot(net_model_dog , rep="best")
```


```{r}
pred_net_dog <- predict(net_model_dog,testing_dog[,-6] )
idx <- apply(pred_net_dog, c(1), maxidx)
pred_net_dog <-as.factor(c('0', '1')[idx])

confusionMatrix(pred_net_dog,testing_dog$outcome_type_adopt)
```
...
confusionMatrix(pred_net_cat,testing_cat$outcome_type_adopt)
```
...
```{r}
plot(net_model_rabbit , rep="best")
```


```{r}
pred_net_rabbit <- predict(net_model_rabbit,testing_rabbit[,-6] )
idx <- apply(pred_net_rabbit, c(1), maxidx)
pred_net_rabbit <-as.factor(c('0', '1')[idx])

confusionMatrix(pred_net_rabbit,testing_rabbit$outcome_type_adopt)
```
